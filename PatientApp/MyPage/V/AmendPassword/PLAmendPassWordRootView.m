...
@interface PLAmendPassWordRootView ()
{
...
@implementation PLAmendPassWordRootView

- (instancetype)initWithFrame:(CGRect)frame withViewModel:(PLAmendPassWordViewModel*)viewModel
{
...
- (void)addViews
{
    ...
    [self.topBaseWhiteView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(singleViewHeight * 4);
    }];
    [self.telephoneField mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self.self.baseScrollView.mas_bottom);
    ...
    [self.baseScrollView setContentSize:CGSizeMake(viewSize.width, singleViewHeight * 4 + tipLabelHeight + 10 + 30 + self.sureButtonView.height + 10)];
...
- (void)clickSureButtonHandler
{
    ...
    [self.viewModel amendPassWordRequest];
}
